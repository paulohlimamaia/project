...
export default {
  ...
  mounted(){
    ...
    http.get('exportacoes/despesas').then(function(response) {
      response.data.forEach(element => {
          self.exercicios.push(element);
      });
    });

    http.get('exportacoes/layouts?tipo=DESPESA').then(function(response) {
      response.data.forEach(element => {
  ...
  methods: {
    ...
    handleGerar(){
      ...
      if(!self.exercicio){
        ...
        http.post('exportacoes/despesas', form).then(function(response){
