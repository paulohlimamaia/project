    ...
    <el-table v-loading="loading" :data="list" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column align="center" label="Ações" width="350">
        <template slot-scope="scope">
          ...
          <el-button v-if="scope.row.roles.includes('visitor') || scope.row.roles.includes('user') || scope.row.roles.includes('editor') || scope.row.roles.includes('manager')" v-permission="['manage user']" type="danger" size="small" icon="el-icon-delete" @click="handleDelete(scope.row.id, scope.row.name);">
            Deletar
          </el-button>
        </template>
      </el-table-column>
